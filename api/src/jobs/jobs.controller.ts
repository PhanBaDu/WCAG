...
export class JobsController {
  ...
  @Get(':slug')
  @ApiOperation({ summary: 'Lấy chi tiết 1 tin tuyển dụng theo slug' })
  @ApiResponse({ status: 200, description: 'Chi tiết việc làm' })
  async findOne(@Param('slug') slug: string): Promise<ApiResponseDto> {
    const data = await this.jobsService.findOneBySlug(slug);
    return ApiResponseDto.success(data, 'Lấy chi tiết thành công');
  }

  @Post()
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Đăng tin tuyển dụng mới (dành cho Employer)' })
  @ApiResponse({ status: 201, description: 'Đăng tin thành công' })
  async create(
    @CurrentUser('userId') userId: string,
    @Body() dto: CreateJobDto
  ): Promise<ApiResponseDto> {
    const data = await this.jobsService.create(userId, dto);
    return ApiResponseDto.success(data, 'Đăng tin thành công. Tin đang chờ duyệt.');
  }
}
